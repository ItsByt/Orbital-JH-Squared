...
describe("UI Helpers", () => {
  describe("getConnector", () => {
    it.each([
      ...
      { index: 0, length: 3, expected: "left-1/2 right-0", scenario: "first item in multi-list" },
      { index: 2, length: 3, expected: "left-0 right-1/2", scenario: "last item in multi-list" },
      { index: 1, length: 3, expected: "left-0 right-0", scenario: "middle item in multi-list" },
    ])("should return '$expected' for $scenario (index $index, length $length)", ({ index, length, expected }) => {
