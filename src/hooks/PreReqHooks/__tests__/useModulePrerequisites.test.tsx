...
describe("useModulePrerequisites Hook", () => {
    ...
    it("should successfully fetch module details and format the tree", async () => {
        ...
        vi.mocked(nusmodsService.getModule).mockResolvedValue(
            mockModuleDetails as unknown as Awaited<ReturnType<typeof nusmodsService.getModule>>
        );
        vi.mocked(treeFormatter.formatTree).mockReturnValue(
            mockFormattedTree as unknown as ReturnType<typeof treeFormatter.formatTree>
        );
    ...
    it("should flag hasNoPrereqs as true when API returns no prereqTree property", async () => {
        ...
        vi.mocked(nusmodsService.getModule).mockResolvedValue({
            moduleCode: "CS1010",
        } as unknown as Awaited<ReturnType<typeof nusmodsService.getModule>>); // no prereqTree
